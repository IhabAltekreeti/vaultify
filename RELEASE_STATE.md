...
- Remote reconciliation checkpoint: CLEAN / CONSISTENT THROUGH R1 STEP 14
...
- Steps 7–14 created, updated, or deleted no Qdrant points.

### Hybrid retrieval
- Golden Cell 19B core algorithm preserved: normalized dense embeddings; BM25 `k1=1.5`, `b=0.75`; one-based ranks; RRF constant `60`; lexical coverage, phrase, year, and quantitative-table bonuses.
- Critical live regression PASS: Apple canonical chunks 609; Tesla canonical chunks 140; Apple `$416,161M` and Tesla `$24,901M` evidence reached top-6.
- Step 10 reran the two critical canonical questions, not every historical Cell 19B benchmark case.
...
- Golden Cell 21A-style document grouping extracted into `services/document_catalog.py`.
- Documents group by `document_hash` when available, otherwise by normalized filename.
- Runtime is generic: entity rules are caller-supplied and unknown customer files use a readable filename-derived fallback.
- Catalog builder fails closed if a supplied chunk belongs to a different tenant.
- Live Step 12 PASS: Apple tenant 745 chunks / 2 documents; Tesla tenant 140 chunks / 1 document.

### Entity-routed hybrid retrieval
- Golden Cell 21C plus active Cell 21C.1 aggregate-metric patch extracted into `services/entity_routing.py`.
- One Dense + BM25 hybrid index is prepared per entity; routes search only the selected entity's registered documents.
- Mixed-corpus live PASS: Apple/Tesla questions routed independently; comparison produced separate routes; `$416,161M` and `$24,901M` evidence reached routed top-6.
- Strict Tesla tenant rejected Apple retrieval; ambiguous/outside-corpus questions stop before retrieval.
...
- Table evidence prefers metric-row / requested-period-column extraction; text-window extraction is the fallback.
- Live Step 14 PASS: Apple `416,161`, Tesla `24,901`, and the comparison were all verified.
- The first Step 14 comparison exposed the missing active Cell 21C.1 `net_sales -> total net sales + net sales` patch; parity was restored before PASS.
...
This does not yet prove live Qdrant/Groq/V2 answer orchestration; that remains a separate integration concern.
...
- `tests/regression/test_evidence_verification.py`
- `notebooks/Vaultify_R1_Control_Panel.ipynb`

## Test-evidence boundary
- Committed pytest covers Flask request-flow security, Query Analyzer V1, document catalog safety, entity-routing control flow, and evidence-verification behavior.
...
- grounded answer generation / clean `answer_question_v2`
